<?php
    ...
    create_table($con, 'imgboard');//자유게시판테이블생성
    create_table($con, 'imgboard_ripple');//자유게시판덧글테이블생성
    ...
    if (isset($_GET["mode"]) && $_GET["mode"] == "search") {
        ...
        $sql = "SELECT * from `imgboard` where $find like '%$q_search%' order by num desc;";
    } else {
...
<html lang="ko" dir="ltr">
    <head>
        ...
        <link rel="stylesheet" type="text/css" href="http://<?= $_SERVER['HTTP_HOST'] ?>/rewhwanhome/imgboard/css/imgboard.css">
                ...
                <div id="col2">
                    ...
                    <div id="list_content">
                        ...
                        for ($i = $start; $i < $start + SCALE && $i < $total_record; $i++) {
                            ...
                            if ($row["file_name_0"])
                                $file_image = "<img src='./data/".$row['file_copied_0']."' style='width:80px; height: 60px; object-fit:cover; overflow:hidden;'>";
                            else
                                $file_image = "<img src='./img/nopicture.png' style='width:80px; height:60px; object-fit: cover;'>";
                            ?>
                            <div id="list_item">
                                <div id="list_item1"><?= $number ?></div>
                                <div id="list_item2">
                                    <?= $file_image ?>
                                    <a href="./view.php?num=<?= $num ?>&page=<?= $page ?>&hit=<?= $hit + 1 ?>" style="margin-left:5px; vertical-align: top; font-size: 18px;"><?= $subject ?></a>

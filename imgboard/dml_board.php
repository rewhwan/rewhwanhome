...
if (isset($_POST["mode"]) && $_POST["mode"] == "insert") {
    ...
    $sql = "INSERT INTO `imgboard` VALUES (null,'$q_userid','$username','','$q_subject','$q_content','$regist_day',0,'$is_html','$upfile_name','$copied_file_name','$type[0]');";
    ...
    $sql = "SELECT num from `imgboard` where id ='$userid' order by num desc limit 1;";
    $result = mysqli_query($con, $sql);
    if (!$result) {
    ...
    $sql = "SELECT `file_copied_0` from `imgboard` where num ='$q_num';";
    $result = mysqli_query($con, $sql);
    if (!$result) {
        alert_back('Error: 6' . mysqli_error($con));
        // die('Error: ' . mysqli_error($con));
    ...
    $sql = "DELETE FROM `imgboard` WHERE num=$q_num";
    $result = mysqli_query($con, $sql);
    if (!$result) {
        die('Error: ' . mysqli_error($con));
    }

    $sql = "DELETE FROM `imgboard_ripple` WHERE parent=$q_num";
    $result = mysqli_query($con, $sql);
    if (!$result) {
        die('Error: ' . mysqli_error($con));
    }
    ...
    if (isset($_POST['del_file']) && $_POST['del_file'] == '1') {
        ...
        $sql = "UPDATE `imgboard` SET `file_name_0`='', `file_copied_0` ='', `file_type_0` =''  WHERE `num`=$q_num;";
        $result = mysqli_query($con, $sql);
        if (!$result) {
            die('Error: ' . mysqli_error($con));
        }
    ...
    if (!empty($_FILES['upfile']['name'])) {
        ...
        $sql = "UPDATE `imgboard` SET `file_name_0`= '$upfile_name', `file_copied_0` ='$copied_file_name', `file_type_0` ='$type[0]' WHERE `num`=$q_num;";
        $result = mysqli_query($con, $sql);
        if (!$result) {
            die('Error: ' . mysqli_error($con));
        }
    ...
    $sql = "UPDATE `imgboard` SET `subject`='$q_subject',`content`='$q_content',`regist_day`='$regist_day',`is_html` ='$is_html'  WHERE `num`=$q_num;";
    $result = mysqli_query($con, $sql);
    if (!$result) {
        die('Error: ' . mysqli_error($con));
    }
    ...
    if (!$rowcount) {
        ...
        $sql = "INSERT INTO `imgboard_ripple` VALUES (null,'$q_parent','$q_userid','$q_username', '','$q_content','$regist_day')";
        $result = mysqli_query($con, $sql);
        if (!$result) {
            die('Error: ' . mysqli_error($con));
        }
    ...
    $sql = "DELETE FROM `imgboard_ripple` WHERE num={$q_num}";
    $result = mysqli_query($con, $sql);
    if (!$result) {
        die('Error: ' . mysqli_error($con));
    }
